...
class Punch_model extends MY_Model
{
    ...
    public function get_file_for_punch()
    {
        ...
        $doc_permissions = $this->db->select('permission_value')->from('tbl_user_permissions')->where(['user_id' => $user_id, 'permission_type' => 'Document'])->get()->result_array();
        $doc_type_ids = array_column($doc_permissions, 'permission_value');
        $loc_permissions = $this->db->select('permission_value')->from('tbl_user_permissions')->where(['user_id' => $user_id, 'permission_type' => 'Location'])->get()->result_array();
        $location_ids = array_column($loc_permissions, 'permission_value');
        $dept_permissions = $this->db->select('permission_value')->from('tbl_user_permissions')->where(['user_id' => $user_id, 'permission_type' => 'Department'])->get()->result_array();
        $department_ids = array_column($dept_permissions, 'permission_value');
        if (empty($doc_type_ids) || empty($location_ids) || empty($department_ids)) {
            return [];
        }
        $this->db->select('*')->from("y{$this->year_id}_scan_file")->where_in('group_id', [$group_id])->where($conditions)->where_in('doc_type_id', $doc_type_ids)->where_in('location_id', $location_ids)->where_in('department_id', $department_ids)->order_by('scan_id', 'desc');
        $query = $this->db->get();
    ...
    function vspl_get_file_for_punch()
    {
        $group_id = $this->session->userdata('group_id');
        $user_id = $this->session->userdata('user_id');
        $doc_type_ids = $this->db->select('permission_value')->from('tbl_user_permissions')->where(['user_id' => $user_id, 'permission_type' => 'Document'])->get()->result_array();
        $location_ids = $this->db->select('permission_value')->from('tbl_user_permissions')->where(['user_id' => $user_id, 'permission_type' => 'Location'])->get()->result_array();
        $department_ids = $this->db->select('permission_value')->from('tbl_user_permissions')->where(['user_id' => $user_id, 'permission_type' => 'Department'])->get()->result_array();
        $doc_type_ids = array_column($doc_type_ids, 'permission_value');
        $location_ids = array_column($location_ids, 'permission_value');
        $department_ids = array_column($department_ids, 'permission_value');
        if (empty($doc_type_ids) || empty($location_ids) || empty($department_ids)) {
            return [];
        }
        $this->db->select('*')->from("y{$this->year_id}_scan_file")->where('is_file_punched', 'Y')->where('is_scan_resend', 'N')->where_in('group_id', $group_id)->where('is_final_submitted', 'Y')->where('finance_punch_action_status', 'N')->where('is_rejected', 'N')->where("((location_id IS NOT NULL AND bill_approval_status = 'Y') OR location_id IS NULL)")->where_in('doc_type_id', $doc_type_ids)->where_in('location_id', $location_ids)->where_in('department_id', $department_ids)->order_by('bill_approved_date', 'DESC');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    {
        $this->db->select(['punchfile.document_number AS DocNo', 'punchfile.finance_punched_date AS Date', "'' AS EmptyField", 'punchfile.account AS CashBankAC', 'master_business_entity.focus_code AS BusinessEntity', 'punchfile.narration AS sNarration', 'punchfile.favouring AS Favouring', 'punchfile.TDS_JV_no AS TDSJVNo', 'master_cost_center.cost_center_name AS CostCenter', 'master_work_location.location_name AS Location', 'master_crop.crop_code AS Crop', 'core_activity.activity_code AS Activity', 'master_state.state_code AS State', 'master_crop_category.crop_category_code AS Category', 'master_region.region_code AS Region', 'master_department.department_code AS Department', 'cash_voucher_items.payment_term AS PMTCategory', 'master_business_unit.business_unit_code AS BusinessUnit', 'master_account_ledger.focus_code AS Account', 'cash_voucher_items.Total_Amount AS TotalAmount', 'cash_voucher_items.ReferenceNo AS Reference', 'cash_voucher_items.Remark AS sRemarks', 'punchfile.TDS_section AS TDS',]);
        ...
        $this->db->where(['y{$this->year_id}_scan_file.group_id' => 16, 'y{$this->year_id}_scan_file.finance_punch' => 'Y', 'y{$this->year_id}_scan_file.is_file_approved' => 'Y', 'y{$this->year_id}_scan_file.finance_punch_status' => 'N', 'punchfile.finance_punched_date IS NOT NULL' => null, 'punchfile.finance_punched_by IS NOT NULL' => null,]);
        if (!empty($doctype)) {
        ...
        if (!empty($from_date)) {
            $this->db->where('punchfile.finance_punched_date >=', $from_date);
        }
        if (!empty($to_date)) {
            $this->db->where('punchfile.finance_punched_date <=', $to_date);
        }
    ...
    public function get_total_rows($doctype = null, $search = null, $from_date = null, $to_date = null)
    {
        ...
        $this->db->join("y{$this->year_id}_scan_file", 'y{$this->year_id}_scan_file.scan_id = cash_voucher_items.scan_id', 'left');
        $this->db->join('punchfile', 'y{$this->year_id}_scan_file.scan_id = punchfile.scan_id', 'left');
        $this->db->where(['y{$this->year_id}_scan_file.group_id' => 16, 'y{$this->year_id}_scan_file.finance_punch' => 'Y', 'y{$this->year_id}_scan_file.finance_punch_status' => 'N', 'punchfile.finance_punched_date IS NOT NULL' => null, 'punchfile.finance_punched_by IS NOT NULL' => null,]);
        if (!empty($doctype)) {
            $this->db->where('y{$this->year_id}_scan_file.doc_type_id', $doctype);
        }
        if (!empty($search)) {
    ...
    public function get_finance_punched_files($user_id, $from_date = null, $to_date = null, $show_all = false)
    {
        ...
        if (!$show_all) {
            if (!empty($from_date) && !empty($to_date)) {
                $this->db->where('DATE(finance_punched_date) >=', $from_date)->where('DATE(finance_punched_date) <=', $to_date);
            } else {
                $this->db->where('DATE(finance_punched_date)', date('Y-m-d'));
            }
        }
        $this->db->order_by('scan_id', 'desc');
        return $this->db->get()->result_array();
