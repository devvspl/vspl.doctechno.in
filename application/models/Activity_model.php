...
class Activity_model extends CI_Model
{
    ...
    {
        $result = $this->db->insert('core_activity', $data);
        if ($result) {
            return true;
        } else {
            return false;
    ...
    public function get_activity_list($id = null)
    {
        ...
        $this->db->from('core_activity a1');
        $this->db->join('core_activity a2', 'a1.activity_group = a2.activity_id', 'left');
    ...
    public function delete($id)
    {
        ...
        $result = $this->db->update('core_activity', array('is_deleted' => 'Y', 'updated_by' => $this->session->userdata('user_id'), 'updated_at' => date('Y-m-d H:i:s')));
        if ($result) {
            return true;
        } else {
            return false;
    ...
    public function update($data)
    {
        ...
        $result = $this->db->update('core_activity', $data);
        if ($result) {
            return true;
        } else {
            return false;
