...
class FirmController extends CI_Controller {
            ...
            if (isset($_FILES['file']['tmp_name']) && is_uploaded_file($_FILES['file']['tmp_name'])) {
                ...
                if (!empty($csvData)) {
                    foreach ($csvData as $row) {
                        $rowCount++;
                        $firmData = array('firm_type' => $row['firm_type'], 'firm_name' => $row['firm_name'], 'firm_code' => $row['firm_code'], 'country_id' => $row['country_id'], 'state_id' => $row['state_id'], 'city_name' => $row['city_name'], 'pin_code' => $row['pin_code'], 'address' => $row['address'], 'gst' => $row['gst'], 'status' => 'A',);
    ...
    public function saveVendor() {
        ...
        $this->form_validation->set_rules('firm_type', 'Firm Type', 'trim|required');
        $this->form_validation->set_rules('firm_name', 'Firm Name', 'trim|required');
        $this->form_validation->set_rules('firm_code', 'Firm Code', 'trim');
        $this->form_validation->set_rules('country_id', 'Country', 'trim|required');
        $this->form_validation->set_rules('state_id', 'State', 'trim|required');
        if ($this->form_validation->run() == false) {
        ...
        $data['status'] = 'R';
        $data['created_by'] = $this->session->userdata('user_id');
        $insert = $this->BaseModel->insertData('master_firm', $data);
        if ($insert) {
            echo json_encode(['status' => true, 'message' => 'Firm saved successfully!']);
        } else {
            echo json_encode(['status' => false, 'message' => 'Failed to save firm.']);
        }
    }
    public function vendorRequest() {
        $this->session->set_userdata('top_menu', 'request');
        $this->session->set_userdata('sub_menu', 'vendor-request');
        $this->data['main'] = 'firm/vendor-request';
        $this->data['vendorRequest'] = $this->Firm_model->getVendorRequests(); 
        $this->load->view('layout/template', $this->data);
    }
    public function approveVendor() {
        $firm_id = $this->input->post('firm_id');
        if (!empty($firm_id)) {
            $updated = $this->db->where('firm_id', $firm_id)->update('master_firm', ['status' => 'A']);
            if ($updated) {
                echo json_encode(['status' => true, 'message' => 'Vendor approved successfully.']);
            } else {
                echo json_encode(['status' => false, 'message' => 'Failed to approve vendor.']);
            }
        } else {
