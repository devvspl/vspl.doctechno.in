...
class Punch extends CI_Controller {
    ...
    private function getInvoiceData($scan_id, $doc_type_id) {
        $table = "y" . $this->year_id . "_punchdata_" . $doc_type_id;
        return ["rec" => $this->customlib->getScanData($scan_id), "punch_detail" => $this->db->get_where($table, ["scan_id" => $scan_id])->row(), "firm" => $this->db->get_where("master_firm", ["status" => "A"])->result_array(), "company_list" => $this->customlib->getCompanyList(), "file_list" => $this->customlib->getFileList(), "ledger_list" => $this->customlib->getLedgerList(), "category_list" => $this->customlib->getCategoryList(), "item_list" => $this->customlib->getItemList(), "locationlist" => $this->customlib->getWorkLocationList(), "temp_punch_detail" => $this->db->get_where("ext_tempdata_{$doc_type_id}", ["scan_id" => $scan_id, ])->row(), "document_number" => "CASH/" . date("y-m") . "/" . str_pad($this->db->where("doctype_id", 23)->where("MONTH(created_at)", date("m"))->where("YEAR(created_at)", date("Y"))->count_all_results($table) + 1, 4, "0", STR_PAD_LEFT), "tdsJvNo" => "TDSCASH/" . date("Y-m", strtotime($this->db->select("created_at")->where("doctype_id", 23)->where("MONTH(created_at)", date("m"))->where("YEAR(created_at)", date("Y"))->order_by("created_at", "DESC")->limit(1)->get($table)->row()->created_at??date("Y-m"))) . "/" . str_pad($this->db->where("doctype_id", 23)->where("MONTH(created_at)", date("m"))->where("YEAR(created_at)", date("Y"))->count_all_results($table) + 1, 4, "0", STR_PAD_LEFT), ];
    }
