...
class Miscellaneous_ctrl extends CI_Controller {
    ...
    public function save_lodging() {
        ...
        if ($isExistingRecord) {
            ...
            if ($submit) {
                $this->db->where('Scan_Id', $data['Scan_Id'])->update('scan_file', array('Is_Rejected' => 'N', 'Reject_Date' => NULL, 'Edit_Permission' => 'N', 'finance_punch' => 'N'));
            } else {
                $this->db->where('Scan_Id', $data['Scan_Id'])->update('scan_file', array('Is_Rejected' => 'N', 'Reject_Date' => NULL, 'Edit_Permission' => 'Y'));
            }
        } else {
            $this->db->insert('punchfile', $data);
            $insert_id = $this->db->insert_id();
    ...
    public function Save_Electricity() {
        ...
        if ($this->customlib->check_punchfile($Scan_Id)) {
            ...
            $this->db->where('FileID', $FileID)->update('sub_punchfile', array('Amount' => '-' . $Amount, 'Comment' => $Remark));
            if ($submit) {
                $this->db->where('Scan_Id', $Scan_Id)->update('scan_file', array('Is_Rejected' => 'N', 'Reject_Date' => NULL, 'Edit_Permission' => 'N', 'finance_punch' => 'N'));
            }
        } else {
            $this->db->insert('punchfile', $data);
            $insert_id = $this->db->insert_id();
